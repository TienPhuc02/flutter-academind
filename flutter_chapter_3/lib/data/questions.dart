import 'package:flutter_chapter_3/models/quiz_question.dart';

const question = [
  QuizQuestion("Quả nào trong số này không phải là loại quả hạch?",
      ["Dâu tây", "Dưa hấu", "Cà chua", "Chuối"]),
  QuizQuestion("Đâu là quốc gia lớn nhất thế giới về diện tích đất?",
      ["Nga", "Trung Quốc", "Mỹ", "Brazil"]),
  QuizQuestion(
      'Ai là người đã lãnh đạo cách mạng dân chủ ở Ấn Độ và được biết đến với danh hiệu "Bố của Quốc dân"?',
      [
        "Mahatma Gandhi",
        " Jawaharlal Nehru",
        "Subhas Chandra Bose",
        "Bhagat Singh"
      ]),
  QuizQuestion(
      " Cuốn tiểu thuyết nào của George Orwell mô tả một xã hội tương lai đầy kiểm soát và giám sát?",
      ["Animal Farm", "1984", "Brave New World", "Fahrenheit 451"]),
  QuizQuestion(
      "Năm 1994, bộ phim nào đã giành giải Oscar cho hạng mục Phim Xuất Sắc Nhất?",
      ["Forrest Gump", "The Shawshank Redemption", "Pulp Fiction", "Titanic"]),
  QuizQuestion('Ban nhạc nào được biết đến với ca khúc "Stairway to Heaven"?',
      ["The Beatles", "Led Zeppelin", " Pink Floyd", " Queen"]),
  QuizQuestion("Ai là cầu thủ đã giành giải Quả Bóng Vàng 2020?",
      ["Cristiano Ronaldo", " Lionel Messi", "Kylian Mbappé", "Neymar Jr."]),
  QuizQuestion(" Công ty nào đã phát triển hệ điều hành iOS?",
      ["Microsoft", " Google", " Apple", " IBM"]),
  QuizQuestion(" Hành tinh nào là hành tinh lớn nhất trong hệ Mặt Trời?",
      ["Mặt Trăng", " Sao Hỏa", " Sao Mộc", " Sao Mộc"]),
  QuizQuestion('Ai là tác giả của cuốn tiểu thuyết "Người Lái Đò"?"', [
    "Ernest Hemingway",
    " Mark Twain",
    " Herman Melville",
    " Joseph Conrad"
  ]),
];
